...
class QueryProcessor:
    ...
    def process_query(self, query):
        if query.type == "check":
            ...
            if (query.type == 'add') and (query.s not in self.elems[hash_val]):
                self.elems[hash_val].insert(0,query.s)
            elif (query.type == 'find'):
                if query.s in self.elems[hash_val]:
                    self.write_search_result(1)
                else:
                    self.write_search_result(0)
            elif (query.type == 'del'):
                try :
                    self.elems[hash_val].remove(query.s)
                except ValueError:
                    pass
            # try:
